...
data ISBN10ValidationError
    ...
    | IllegalCharactersInBody -- ^ The body of the ISBN-10 input contains non-numeric characters
    | IllegalCharacterInCheckDigit -- ^ The check digit of the ISBN-10 is not a valid character
    | InvalidCheckDigit -- ^ The check digit is incorrect
    deriving (Show, Eq)
...
{-| Allows for the generation of ISBN-10 values without any validation
-}
      ...
      where
        go n charList acc =
            case charList of
              ...
              c:clist -> go (n - 1) clist (acc + isbnCharToNumericValue c * n)
